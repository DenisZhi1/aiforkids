...
const CERTIFICATE_TEMPLATES = {
  "17": {
    title: "Редактор сертификата С17",
    subtitle: "Шаблон для участников с 17-19 выполненными заданиями",
    imagePath: "cert/17.png",
    storageKey: `cert-editor-${CERTIFICATE_EDITOR_STATE_VERSION}-17`,
    defaults: {
      nameX: 1379,
      nameY: 935,
  ...
  "20": {
    ...
    subtitle: "Шаблон для участников с 20 выполненными заданиями",
    imagePath: "cert/20.png",
    storageKey: `cert-editor-${CERTIFICATE_EDITOR_STATE_VERSION}-20`,
    defaults: {
      nameX: 1379,
      nameY: 935,
      nameFontFamily: '"Sofia Sans Condensed", sans-serif',
...
const templateType = normalizeCertificateType(hashParams.get("type"))
  || normalizeCertificateType(params.get("type"))
  || normalizeCertificateType(payloadData.type)
  || "17";
const templateConfig = CERTIFICATE_TEMPLATES[templateType];
const initialName = resolveInitialName();

const state = {
  ...templateConfig.defaults,
  ...loadStoredSettings(templateConfig.storageKey),
  name: initialName || "Имя Фамилия",
  date: formatToday()
};
...
function ensureValidFontFamily() {
  ...
  if (!isKnownFont) {
    state.nameFontFamily = templateConfig.defaults.nameFontFamily;
  }
}

function ensureValidWeights() {
  state.nameFontWeight = String(clamp(Number(state.nameFontWeight) || Number(templateConfig.defaults.nameFontWeight), FONT_WEIGHT_MIN, FONT_WEIGHT_MAX));
  state.dateFontWeight = String(clamp(Number(state.dateFontWeight) || Number(templateConfig.defaults.dateFontWeight), FONT_WEIGHT_MIN, FONT_WEIGHT_MAX));
}
...
function updateHeader() {
  editorTitle.textContent = templateConfig.title;
  editorSubtitle.textContent = `${templateConfig.subtitle}. Имя из карточки переносится автоматически.`;
}

function resolveInitialName() {
  const nameFromPayload = typeof payloadData.name === "string" ? payloadData.name.trim() : "";
  if (nameFromPayload) return nameFromPayload;

  const nameFromHash = (hashParams.get("name") || "").trim();
  if (nameFromHash) return nameFromHash;
  ...
  try {
    ...
    if (parsed.type !== templateType) return "";
...
function loadTemplate() {
  ...
  templateImage.src = templateConfig.imagePath;
}
...
function renderAndPersist() {
  saveStoredSettings(templateConfig.storageKey, {
    nameX: state.nameX,
    nameY: state.nameY,
...
async function downloadCertificate() {
  ...
  try {
    ...
    const safeName = slugify(state.name || `certificate-${templateType}`);
    const downloadName = `certificate-${templateType}-${safeName || "student"}.png`;
