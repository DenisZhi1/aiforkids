...
function updateLessonTransition(dt) {
  ...
  if (lessonIndex + 1 >= lessonDeck.length) {
    shuffleLessonDeck(getCurrentLesson().id);
  } else {
    lessonIndex += 1;
  }
  lessonProgress = 0;
...
function shuffleLessonDeck(avoidFirstId = "") {
  const nextDeck = LESSON_BANK.slice();
  ...
  lessonDeck = nextDeck;
  lessonIndex = 0;
  lastStartingLessonId = nextDeck[0]?.id || "";
}
...
function update(dt) {
  ...
  const normalSpeed = Math.min(MAX_RUN_SPEED, BASE_RUN_SPEED + state.elapsed * .085 + state.acorns * .014);
